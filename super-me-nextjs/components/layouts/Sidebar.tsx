    ...
    return (
        <div className='w-60 h-screen flex flex-col justify-between py-5 px-6 items-center bg-sidebar-bg text-white'>
            <div className=" flex flex-col">
                <Link href="/" className='flex items-center gap-4 mb-8 cursor-pointer' >
                    <Image src={superMelogo} alt="SuperMe" className='h-10 w-10 rounded-lg' />
                ...
                <nav className="">
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link key={item.name} href={item.path} onClick={() => sidebarHandler(item.name)} className={`flex h-15 w-50 p-4 items-center gap-4 cursor-pointer rounded-lg ${activeItem === item.name ? 'bg-[#4C41CC]' : 'bg-transparent text-slate-300 hover:bg-accent-purple hover:text-slate-50 hover:rounded-lg'}  `} >
                                <ItemIcon className='' />
                                <span >{item.name}</span>
                            </Link>
                        )
